...
public class MathVector {
	...
	private double scalarProd(MathVector vector2) {
		double scalarProd = 0;
		for( int i = 0; i < this.length;i++) {
	...
	public int intersectionAngle(MathVector vector2) {
		double cosTheta = this.scalarProd(vector2) / (this.vectorSize() * vector2.vectorSize());
	    return (int) Math.toDegrees(Math.acos(cosTheta));
	}
}
